...
#define NETWOR_BUFFER_SIZE 1024
...
#define RART_LOG_TRACE		3 //BOOST_LOG_LEVEL_3
#define RART_LOG_DEBUG		4 //BOOST_LOG_LEVEL_4
...
#define TRACE( level, _trace )	                                            \
		if(g_nLogLevel >= level){	\
			...
			if(level == RART_LOG_ERROR) \
				...
				mtkwarn("[%s %ld] %s\n", p.leaf().c_str(), __LINE__, ss.str().c_str()); \
			else if(level == RART_LOG_TRACE) \
				mtkwarn("[%s %ld] %s\n", p.leaf().c_str(), __LINE__, ss.str().c_str()); \
			else if(level == RART_LOG_DEBUG)\
				mtklog("[%s %ld] %s\n", p.leaf().c_str(), __LINE__, ss.str().c_str()); \
 		}
#else
#define TRACE( level, _trace )	                                            \
